# 🛍️ ai-shop Professional Edition v4.2.0
...
<p align="center">
  <strong>ai-shop Professional Edition v4.2.0</strong>
</p>
...
نسخه 4.2.0 پنجره ساده Basic Auth مرورگر را با یک صفحه ورود فارسی و واکنش‌گرا جایگزین می‌کند:
...
در نسخه 4.2.0 رمز ثابت پنل از طریق ربات نمایش داده نمی‌شود. هر بار مدیر روی گزینه زیر بزند:
...
نسخه 4.2.0 این موارد را اضافه می‌کند:
...
# رفع Rollback ناشی از Nginx در نسخه 4.2.0
...
علت، تکرار دستور `proxy_http_version 1.1` در قالب Nginx بود. نسخه 4.2.0:
...
بعد از قرار دادن نسخه 4.2.0 در GitHub:
...
# رفع مشکل صادر نشدن رمز پنل در نسخه 4.2.0
...
نسخه 4.2.0 علاوه بر رفع Import، رمزها را در PostgreSQL ذخیره می‌کند:
...
# رفع خطای 502 بعد از ورود در نسخه 4.2.0
...
نسخه 4.2.0 داده‌های موجودی و تنظیمات را پیش از ساخت صفحه دریافت می‌کند و خطاهای احتمالی پنل را به یک صفحه فارسی با شناسه پیگیری تبدیل می‌کند؛ بنابراین سوکت برنامه بدون پاسخ قطع نمی‌شود.
...
# نسخه 4.2.0 — فروشگاه اکانت هوش مصنوعی
...
# نسخه 4.2.0 — مدیریت کامل داخل تلگرام
...
# نسخه 4.2.0 — مدیریت درگاه و تحویل دستی

مدیر می‌تواند از داخل ربات روش‌های پرداخت را تعریف و مدیریت کند:

- کارت‌به‌کارت
- زرین‌پال
- آیدی‌پی، نکست‌پی، پی‌پینگ و زیبال
- لینک پرداخت اختصاصی ایرانی
- PayPal و Stripe Payment Link
- Perfect Money
- USDT و سایر رمزارزها
- هر روش پرداخت سفارشی با نام، ارز، لینک، اطلاعات حساب و راهنما

زرین‌پال از اتصال API موجود پروژه استفاده می‌کند. سایر درگاه‌ها به‌صورت لینک پرداخت یا پرداخت دستی و دریافت رسید فعال هستند. اتصال مستقیم API هر پرداخت‌یار نیازمند کلید رسمی و آداپتور اختصاصی همان سرویس است.

## تحویل دستی سرویس

پس از تأیید پرداخت، سفارش وارد وضعیت «در انتظار تحویل مدیر» می‌شود. مدیر از مسیر زیر متن تحویل را وارد می‌کند:
...
متن شامل ایمیل، رمز، لینک فعال‌سازی، کد لایسنس و راهنما می‌تواند باشد. ربات آن را برای خریدار ارسال و سفارش را تحویل‌شده ثبت می‌کند.


---

# نسخه 4.2.0 — شماره تلفن و ایموجی پریمیوم

- دریافت شماره با دکمه استاندارد «ارسال شماره من»
- پشتیبانی از شماره‌های 0912... و +98912...
- جلوگیری از ثبت Contact متعلق به کاربر دیگر
- ذخیره custom_emoji_id برای دسته‌بندی‌ها
- پذیرش ایموجی معمولی، ایموجی پریمیوم و شناسه custom_emoji_id
- استفاده از icon_custom_emoji_id در دکمه‌های سازگار
