# 🛍️ ai-shop Professional Edition v3.4.1
...
<p align="center">
  <strong>ai-shop Professional Edition v3.4.1</strong>
</p>
...
نسخه 3.4.1 پنجره ساده Basic Auth مرورگر را با یک صفحه ورود فارسی و واکنش‌گرا جایگزین می‌کند:
...
در نسخه 3.4.1 رمز ثابت پنل از طریق ربات نمایش داده نمی‌شود. هر بار مدیر روی گزینه زیر بزند:
...
نسخه 3.4.1 این موارد را اضافه می‌کند:
...
# رفع Rollback ناشی از Nginx در نسخه 3.4.1
...
علت، تکرار دستور `proxy_http_version 1.1` در قالب Nginx بود. نسخه 3.4.1:
...
بعد از قرار دادن نسخه 3.4.1 در GitHub:
...
# رفع مشکل صادر نشدن رمز پنل در نسخه 3.4.1
...
نسخه 3.4.1 علاوه بر رفع Import، رمزها را در PostgreSQL ذخیره می‌کند:
...
# رفع خطای 502 بعد از ورود در نسخه 3.4.1
...
نسخه 3.4.1 داده‌های موجودی و تنظیمات را پیش از ساخت صفحه دریافت می‌کند و خطاهای احتمالی پنل را به یک صفحه فارسی با شناسه پیگیری تبدیل می‌کند؛ بنابراین سوکت برنامه بدون پاسخ قطع نمی‌شود.
